...
```bash
DataProcessing$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
DataProcessing$ sam local start-api
DataProcessing$ curl http://localhost:3000/
```
...
```bash
DataProcessing$ sam logs -n HelloWorldFunction --stack-name DataProcessing --tail
```
...
```bash
sam delete --stack-name DataProcessing
```
